...
- 可观测性
  - 仪表板实时指标
  - SQLite 历史指标与请求日志
  - 系统参数统计模块（Linux/Windows）：CPU、内存、Swap、网络、磁盘吞吐、TCP 状态、进程/文件句柄、运行时长的实时与历史图表
  - 实时日志面板
- 内置测试工具（UI）
  - HTTP 测试
  - 路由匹配测试 + 测试套件
...
- 触发方式：GitHub Actions -> `Manual Build (Single Platform)` -> `Run workflow`
- 输入参数 `platform`：
  - `windows-x64`
  - `linux-amd64`
  - `macos-arm64`
  - `macos-x64`
- 输入参数 `publish_release`：
  - `true`：上传安装包到 GitHub Release，并覆盖同名资源（`--clobber`）
  - `false`：只构建，产物保留在 workflow artifacts
- 输入参数 `release_tag`：
  - 可选。留空时自动使用 `v<Cargo.toml 中的 version>`
...
- `system_metrics_sample_interval_secs`
  - 采样间隔（秒），范围 `1..300`
  - 默认值：`10`
  - Windows 下为降低采集开销，实际采样间隔最小会限制为 `3` 秒
- `system_metrics_persistence_enabled`
  - 控制系统参数点位是否写入 `system_metrics` 表
  - 该开关只影响系统参数统计持久化，不会修改全局指标持久化设置
- 运行行为：
  - 图表渲染和实时推送仅在“系统参数统计”页面打开并订阅后进行
  - 当 `metrics_storage.enabled = true` 且 `system_metrics_persistence_enabled = true` 时，即使页面未打开，后端也会持续采样并写库
  - 若全局 `metrics_storage` 未启用，则系统参数持久化不会生效

### 指标存储（`[metrics_storage]`）
...
- `metrics_storage` 为可选配置；缺失或 `enabled = false` 时，历史写库与查询不可用。
