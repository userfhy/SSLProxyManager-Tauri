      ...
      <div class="top-bar-content">
        ...
        <div class="top-bar-right">
          <div class="theme-control">
            <el-dropdown
              trigger="click"
              popper-class="font-size-dropdown"
            ...
            >
              <span>
                ...
                >
                  ...
                  {{ saving ? $t("app.saving") : $t("app.saveConfig") }}
                </el-button>
              </span>
            </el-tooltip>
          </div>
    ...
    <div class="main-content">
      ...
      <div class="content-area">
        <BaseConfig
          ...
          ref="baseConfigRef"
        />
        <ConfigCard v-show="activeTab === 'config'" ref="configCardRef" />
        <WsProxyConfig
        ...
        <About v-if="isLazyTabLoaded('about')" v-show="activeTab === 'about'" ref="aboutRef" />
      </div>
    </div>
...
const loadFontSize = () => {
  ...
  const isAllowed =
    saved !== null && fontSizeOptions.includes(Number(saved) as (typeof fontSizeOptions)[number]);
...
const handleFontSizeCommand = (command: string | number) => {
  ...
  const isAllowed =
    value === "default" ||
    fontSizeOptions.includes(Number(value) as (typeof fontSizeOptions)[number]);
  if (!isAllowed) return;
...
const handleQuit = () => {
  ElMessageBox.confirm(t("app.confirmQuit"), t("app.quit"), {
...
const initializeAppCore = async () => {
  ...
  await EventsOn("update-check-result", (result: any) => {
    if (result?.has_update && result?.update_info) {
      ...
      ElMessageBox.confirm(message, t("app.updateAvailable"), {
...
.theme-btn:hover {
  ...
  box-shadow:
    0 0 0 2px rgba(79, 156, 249, 0.28),
    0 8px 18px rgba(79, 156, 249, 0.24);
}
