...
<template>
  <el-card
    class="config-card config-page"
    shadow="hover"
    header-class="config-main-header proxy-config-main-header"
  >
    <template #header>
      ...
      >
        <el-card class="rule-card" shadow="hover">
          ...
          <el-form :model="rule" label-width="110px" size="small" class="form-grid">
                ...
                >
                  <template #header>
                    <div class="route-header">
                      <div class="route-title">
                        {{ $t("configCard.route") }} {{ routeIndex + 1 }}
                      </div>
                      <div style="display: flex; align-items: center; gap: 12px">
                        <el-switch
                          v-model="rt.Enabled"
                          @change="() => onToggleRouteEnabled(rule, rt)"
                        />
                        <el-button
                    ...
                    <el-row :gutter="20" class="route-match">
                      <el-col :span="10">
                        <el-form-item :label="$t('configCard.host')">
                          <el-input
                            v-model="rt.Host"
                            :placeholder="$t('configCard.hostPlaceholder')"
                          />
                      ...
                      <el-col :span="10">
                        <el-form-item :label="$t('configCard.pathPrefix')">
                          <el-input
                            v-model="rt.Path"
                            :placeholder="$t('configCard.pathPlaceholder')"
                          />
                        </el-form-item>
                      </el-col>
                    </el-row>
                        ...
                        <el-form-item :label="$t('configCard.matchHeaders')">
                          <div class="match-headers-section">
                            <TransitionGroup name="list" tag="div">
                              <div
                                v-for="(kv, hIndex) in rt.MatchHeadersList || []"
                                :key="hIndex"
                                class="header-item"
                              >
                                <el-input
                                  v-model="kv.Key"
                                  :placeholder="$t('configCard.headerKeyPlaceholder')"
                                />
                                <el-input
                                  v-model="kv.Value"
                                  :placeholder="$t('configCard.headerValuePlaceholder')"
                                />
                                <el-button
                                  @click="(rt.MatchHeadersList || []).splice(hIndex, 1)"
                                  type="danger"
                                  size="small"
                                  >{{ $t("configCard.delete") }}</el-button
                                >
                              </div>
                            </TransitionGroup>
                            <el-button
                              @click="(rt.MatchHeadersList ||= []).push({ Key: '', Value: '' })"
                              type="primary"
                              size="small"
                    ...
                    <el-row :gutter="22">
                      <el-col :span="10">
                        <el-form-item :label="$t('configCard.proxyPassPath')">
                          <el-input
                            v-model="rt.ProxyPassPath"
                            :placeholder="$t('configCard.proxyPassPathPlaceholder')"
                          />
                          <el-text type="info" size="small" class="mini-hint">
                        ...
                        <el-form-item
                          :label="$t('configCard.followRedirects')"
                      ...
                      <el-col :span="10">
                        <el-form-item :label="$t('configCard.staticDir')">
                          <div class="file-selector">
                            <el-input
                              v-model="rt.StaticDir"
                              :placeholder="$t('configCard.staticDirPlaceholder')"
                              readonly
                            />
                          </div>
                          <el-text type="info" size="small" class="mini-hint">
                      ...
                      <el-col :span="2">
                        <el-button
                          @click="selectDirectory(ruleIndex, routeIndex)"
                          size="small"
                          type="primary"
                          :icon="Folder"
                        >
                  ...
                  <div class="sub-section">
                    ...
                    <div class="sub-section-body">
                      <TransitionGroup name="list" tag="div">
                        <div
                          v-for="(upstream, index) in rt.Upstreams"
                          :key="index"
                          class="upstream-item"
                        >
                          <el-input
                            ...
                            :placeholder="
                              index === 0 ? 'http://127.0.0.1:8080' : 'https://example.com'
                            "
                          />
                          <el-input-number
                          ...
                          <el-button
                            ...
                            size="small"
                            :disabled="
                              rt.Upstreams.length <= 1 &&
                              !(rt.StaticDir && rt.StaticDir.trim() !== '')
                            "
                          >
                            {{ $t("configCard.delete") }}
                          </el-button>
                      ...
                      <el-button
                        @click="addUpstream(ruleIndex, routeIndex)"
                        type="primary"
                        style="margin-top: 12px"
                      >
                  ...
                  <div class="sub-section">
                    ...
                    <div class="sub-section-body">
                      ...
                      <TransitionGroup name="list" tag="div">
                        <div
                          v-for="(kv, hIndex) in rt.SetHeadersList || []"
                          :key="hIndex"
                          class="header-item"
                        >
                          <el-input
                          ...
                          />
                          <el-button
                            @click="(rt.SetHeadersList || []).splice(hIndex, 1)"
                            type="danger"
                            size="small"
                            >{{ $t("configCard.delete") }}</el-button
                          >
                      ...
                      <el-button
                        @click="(rt.SetHeadersList ||= []).push({ Key: '', Value: '' })"
                        type="primary"
                        size="small"
                        style="margin-top: 12px"
                      >
                  ...
                  <div class="sub-section">
                    <div class="sub-section-header">
                      {{ $t("configCard.requestResponseModify") }}
                      <el-alert type="warning" :closable="false" style="width: 50%">
                        <template #title>
                        ...
                        <template #default>
                          <div style="font-size: 13px; line-height: 1.5">
                            ...
                            <br />• {{ $t("configCard.legalWarningItem1") }} <br />•
                            {{ $t("configCard.legalWarningItem2") }} <br />•
                            {{ $t("configCard.legalWarningItem3") }} <br />•
                            {{ $t("configCard.legalWarningItem4") }}
                          </div>
                        </template>
                      </el-alert>
                    </div>
                    <div class="sub-section-body">
                      ...
                      <el-form-item :label="$t('configCard.urlRewrite')">
                        <TransitionGroup name="list" tag="div">
                          <div
                            v-for="(rule, idx) in rt.UrlRewriteRules || []"
                            :key="idx"
                            class="rewrite-rule-item"
                          >
                            <el-input
                              v-model="rule.Pattern"
                              :placeholder="$t('configCard.urlRewritePattern')"
                              style="flex: 1"
                            />
                            <el-input
                              v-model="rule.Replacement"
                              :placeholder="$t('configCard.urlRewriteReplacement')"
                              style="flex: 1"
                            />
                            <el-switch v-model="rule.Enabled" />
                            <el-button
                              @click="(rt.UrlRewriteRules || []).splice(idx, 1)"
                              type="danger"
                              size="small"
                              >{{ $t("configCard.delete") }}</el-button
                            >
                      ...
                      <el-form-item :label="$t('configCard.requestBodyReplace')">
                        <TransitionGroup name="list" tag="div">
                          <div
                            v-for="(rule, idx) in rt.RequestBodyReplace || []"
                            :key="idx"
                            class="replace-rule-item"
                          >
                            <el-input
                              v-model="rule.Find"
                              :placeholder="$t('configCard.findText')"
                              style="flex: 1"
                            />
                            <el-input
                              v-model="rule.Replace"
                              :placeholder="$t('configCard.replaceWith')"
                              style="flex: 1"
                            />
                            <el-select
                            ...
                            <el-checkbox v-model="rule.UseRegex">{{
                              $t("configCard.regex")
                            }}</el-checkbox>
                            <el-switch v-model="rule.Enabled" />
                            <el-button
                              @click="(rt.RequestBodyReplace || []).splice(idx, 1)"
                              type="danger"
                              size="small"
                              >{{ $t("configCard.delete") }}</el-button
                            >
                      ...
                      <el-form-item :label="$t('configCard.responseBodyReplace')">
                        <TransitionGroup name="list" tag="div">
                          <div
                            v-for="(rule, idx) in rt.ResponseBodyReplace || []"
                            :key="idx"
                            class="replace-rule-item"
                          >
                            <el-input
                            ...
                            <el-switch v-model="rule.Enabled" />
                            <el-button
                              @click="(rt.ResponseBodyReplace || []).splice(idx, 1)"
                              type="danger"
                              size="small"
                              >{{ $t("configCard.delete") }}</el-button
                            >
                      ...
                      <el-form-item :label="$t('configCard.removeHeaders')">
                        <TransitionGroup name="list" tag="div">
                          <div
                            v-for="(header, idx) in rt.RemoveHeaders || []"
                            :key="idx"
                            class="header-item"
                          >
                            <el-input
                              v-model="(rt.RemoveHeaders || [])[idx]"
                              :placeholder="$t('configCard.headerNamePlaceholder')"
                            />
                            <el-button
                              @click="(rt.RemoveHeaders || []).splice(idx, 1)"
                              type="danger"
                              size="small"
                              >{{ $t("configCard.delete") }}</el-button
                            >
            ...
            <el-form-item>
              <el-checkbox v-model="rule.SSLEnable">{{
                $t("configCard.enableSSLForRule")
              }}</el-checkbox>
            </el-form-item>
            ...
            <el-form-item>
              <el-checkbox v-model="rule.BasicAuthEnable">{{
                $t("configCard.enableBasicAuth")
              }}</el-checkbox>
            </el-form-item>
            ...
            <template v-if="rule.BasicAuthEnable">
              ...
              <el-form-item :label="$t('configCard.password')">
                <el-input
                  v-model="rule.BasicAuthPassword"
                  type="password"
            ...
            <el-form-item>
              <el-checkbox v-model="rule.RateLimitEnabled">{{
                $t("configCard.enableRateLimit")
              }}</el-checkbox>
...
const updateActiveRuleFromScroll = () => {
  const anchors = Array.from({ length: rules.value.length }, (_, i) =>
    document.getElementById(ruleAnchorId(i)),
...
const onToggleListenRuleEnabled = async (rule: ListenRule) => {
  try {
    ...
    ElMessage.success(
      next ? t("configCard.listenRuleEnabled") : t("configCard.listenRuleDisabled"),
    );
  } catch (error: any) {
  ...
  if (Array.isArray(configData.rules) && configData.rules.length > 0) {
    rules.value = configData.rules.map((rule: any) => {
      const routes = (rule.routes || []).map((rt: any) => ({
        ...
        MatchHeadersList:
          (rt.headers &&
            Object.entries(rt.headers).map(([Key, Value]) => ({
              Key,
              Value: String(Value ?? ""),
            }))) ||
          [],
      ...
      return {
        ...
        ListenAddrs:
          Array.isArray(rule.listen_addrs) && rule.listen_addrs.length > 0
            ? rule.listen_addrs
        ...
        RateLimitEnabled:
          rule.rate_limit_enabled !== undefined ? !!rule.rate_limit_enabled : undefined,
        RateLimitRequestsPerSecond:
          rule.rate_limit_requests_per_second !== undefined
            ? Number(rule.rate_limit_requests_per_second)
            : undefined,
        RateLimitBurstSize:
          rule.rate_limit_burst_size !== undefined ? Number(rule.rate_limit_burst_size) : undefined,
        RateLimitBanSeconds:
          rule.rate_limit_ban_seconds !== undefined
            ? Number(rule.rate_limit_ban_seconds)
            : undefined,
        Routes:
          routes.length > 0
            ? routes
...
const getConfig = () => {
  ...
  const cleanedRules: ListenRule[] = currentRules.map((rule) => ({
    ...
    RateLimitRequestsPerSecond:
      rule.RateLimitRequestsPerSecond !== undefined
        ? Number(rule.RateLimitRequestsPerSecond)
        : undefined,
    RateLimitBurstSize:
      rule.RateLimitBurstSize !== undefined ? Number(rule.RateLimitBurstSize) : undefined,
    RateLimitBanSeconds:
      rule.RateLimitBanSeconds !== undefined ? Number(rule.RateLimitBanSeconds) : undefined,
    Routes: rule.Routes.map((rt) => {
      ...
      return {
        ...
        UrlRewriteRules: (rt.UrlRewriteRules || [])
        ...
        RequestBodyReplace: (rt.RequestBodyReplace || [])
        ...
        ResponseBodyReplace: (rt.ResponseBodyReplace || [])
          .filter((r) => r.Find.trim() !== "")
          .map((r) => ({
            Find: r.Find.trim(),
            Replace: r.Replace.trim(),
  ...
  for (let i = 0; i < cleanedRules.length; i++) {
    ...
    for (let j = 0; j < rule.Routes.length; j++) {
      ...
      if (!hasUpstreams && !hasStaticDir) {
        throw new Error(
          t("configCard.routeNoUpstreamOrStatic", { ruleIndex: i + 1, routeIndex: j + 1 }),
        );
      }
    }
  ...
  const mappedRules = cleanedRules.map((r: any) => ({
    ...
    rate_limit_requests_per_second:
      r.RateLimitRequestsPerSecond !== undefined ? Number(r.RateLimitRequestsPerSecond) : undefined,
    rate_limit_burst_size:
      r.RateLimitBurstSize !== undefined ? Number(r.RateLimitBurstSize) : undefined,
    rate_limit_window_seconds:
      r.RateLimitWindowSeconds !== undefined ? Number(r.RateLimitWindowSeconds) : 1,
    rate_limit_ban_seconds:
      r.RateLimitBanSeconds !== undefined ? Number(r.RateLimitBanSeconds) : undefined,
    routes: (r.Routes || []).map((rt: any) => {
      ...
      return {
        ...
        // 新增字段映射
        methods:
          Array.isArray(rt.Methods) && rt.Methods.length > 0
            ? rt.Methods.map((m: string) => m.trim().toUpperCase()).filter((m: string) => m)
            : undefined,
        headers: Object.keys(headersObj).length > 0 ? headersObj : undefined,
        url_rewrite_rules: (rt.UrlRewriteRules || [])
          .filter((r: any) => r.Pattern.trim() !== "")
          .map((r: any) => ({
            pattern: r.Pattern.trim(),
            replacement: r.Replacement.trim(),
            enabled: r.Enabled !== undefined ? !!r.Enabled : true,
          })),
        request_body_replace: (rt.RequestBodyReplace || [])
          .filter((r: any) => r.Find.trim() !== "")
          .map((r: any) => ({
            find: r.Find.trim(),
            replace: r.Replace.trim(),
            ...
            enabled: r.Enabled !== undefined ? !!r.Enabled : true,
            content_types:
              Array.isArray(r.ContentTypes) && r.ContentTypes.length > 0
                ? r.ContentTypes.map((s: string) => (s || "").trim())
                    .filter((s: string) => s)
                    .join(",")
                : undefined,
          })),
        response_body_replace: (rt.ResponseBodyReplace || [])
          .filter((r: any) => r.Find.trim() !== "")
          .map((r: any) => ({
            find: r.Find.trim(),
            replace: r.Replace.trim(),
        ...
        remove_headers: (rt.RemoveHeaders || [])
          .filter((h: any) => h.trim() !== "")
          .map((h: any) => h.trim()),
        upstreams: (rt.Upstreams || []).map((u: any) => ({
          url: u.URL,
