    ...
    <template #header>
      <div class="header-row">
        ...
        <div class="header-actions">
          <el-switch
            v-model="enabled"
            :active-text="$t('streamProxy.enable')"
            :inactive-text="$t('streamProxy.disable')"
          />
        </div>
      </div>
    </template>
                ...
                <el-row :gutter="10">
                  ...
                  <el-col :span="2">
                    <el-button
                      type="danger"
                      size="small"
                      :disabled="up.servers.length <= 1"
                      @click="removeUpstreamServer(upIndex, svIndex)"
                    >
                      {{ $t("streamProxy.delete") }}
                    </el-button>
                  </el-col>
        ...
        >
          ...
          <el-form :model="sv" label-width="200px">
            ...
            <el-form-item :label="$t('streamProxy.listenAddr')">
              <el-input
                v-model="sv.listen_addr"
                :placeholder="$t('streamProxy.listenAddrPlaceholder')"
            ...
            <el-form-item :label="$t('streamProxy.udp')">
              <el-switch
                v-model="sv.udp"
                active-text="UDP"
                :inactive-text="$t('streamProxy.tcp')"
              />
            </el-form-item>
            ...
            <el-form-item :label="$t('streamProxy.proxyConnectTimeout')">
              <el-input
                v-model="sv.proxy_connect_timeout"
                placeholder="300s"
  ...
  if (enabled.value) {
    ...
    for (let i = 0; i < cleanedUpstreams.length; i++) {
      ...
      for (let j = 0; j < u.servers.length; j++) {
        ...
        if (!isValidHostPort(sv.addr)) {
          throw new Error(
            `Stream Upstream ${i + 1}（${u.name}）/ Server ${j + 1}：addr 格式错误（需要 host:port）：${sv.addr}`,
          );
        }
      }
    }
    ...
    for (let i = 0; i < cleanedServers.length; i++) {
      ...
      if (!names.has(sv.proxy_pass)) {
        throw new Error(
          `Stream Server ${i + 1}：proxy_pass 引用了不存在的 upstream：${sv.proxy_pass}`,
        );
      }
      if (!sv.listen_addr) {
      ...
      if (!parsedPort) {
        throw new Error(
          `Stream Server ${i + 1}：listen_addr 格式错误（示例：127.0.0.1:50002 或 [::]:50002）`,
        );
      }
      ...
      if (!sv.udp) {
        ...
        if (!okTimeout(sv.proxy_connect_timeout)) {
          throw new Error(
            `Stream Server ${i + 1}：proxy_connect_timeout 格式不正确：${sv.proxy_connect_timeout}`,
          );
        }
