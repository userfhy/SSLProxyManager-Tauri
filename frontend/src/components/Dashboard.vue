      ...
      <div class="controls">
        <el-form-item :label="$t('dashboard.listenAddr')" style="margin-bottom: 0">
          <el-select v-model="selectedListen" style="width: 200px">
            <el-option
              v-for="a in listenAddrs"
              :key="a"
              :label="getListenAddrLabel(a)"
              :value="a"
            />
          </el-select>
        </el-form-item>
          ...
          <div class="phase-grid">
            ...
            <div class="phase-item">
              ...
              <div class="phase-line">
                avg {{ formatPhaseMs(phaseTiming?.upstream?.avg_ms) }} ms
              </div>
              <div class="phase-line">
                p95 {{ formatPhaseMs(phaseTiming?.upstream?.p95_ms) }} ms
              </div>
              <div class="phase-line">
                p99 {{ formatPhaseMs(phaseTiming?.upstream?.p99_ms) }} ms
              </div>
            </div>
          </div>
        </div>
        ...
        <template #header>
          <div class="panel-header">
            ...
            <el-button
              ...
              @click="openChartPreview($t('dashboard.qpsTrend'), 'qps')"
            >
        ...
        <template #header>
          <div class="panel-header">
            ...
            <el-button
              ...
              size="small"
              class="panel-preview-btn"
              @click="openChartPreview($t('dashboard.statusDistribution'), 'status')"
            >
              {{ $t("common.fullscreenPreview") }}
            </el-button>
          </div>
        ...
        <template #header>
          <div class="panel-header">
            ...
            <el-button
              ...
              size="small"
              class="panel-preview-btn"
              @click="openChartPreview($t('dashboard.latencyTrend'), 'latency')"
            >
              {{ $t("common.fullscreenPreview") }}
            </el-button>
          </div>
        ...
        <template #header>
          <div class="panel-header">
            ...
            <el-button
              ...
              size="small"
              class="panel-preview-btn"
              @click="openChartPreview($t('dashboard.percentile'), 'percentile')"
            >
              {{ $t("common.fullscreenPreview") }}
            </el-button>
          </div>
        ...
        <template #header>
          <div class="panel-header">
            ...
            <el-button
              ...
              class="panel-preview-btn"
              @click="
                openChartPreview($t('dashboard.upstreamRequestDistributionTop20'), 'upstream')
              "
            >
              {{ $t("common.fullscreenPreview") }}
            </el-button>
        ...
        <div class="tables">
          ...
          <el-card class="table" shadow="never">
            ...
            <div class="rows">
              ...
              <el-empty
                v-if="topUpstreamErrors.length === 0"
                :description="$t('dashboard.noData')"
                :image-size="60"
              />
            </div>
        ...
        <template #header>
          <div class="panel-header">
            ...
            <el-button
              ...
              size="small"
              class="panel-preview-btn"
              @click="openChartPreview($t('dashboard.errorSuccessRateTrend'), 'rate')"
            >
              {{ $t("common.fullscreenPreview") }}
            </el-button>
          </div>
        ...
        <template #header>
          <div class="panel-header">
            ...
            <el-button
              ...
              size="small"
              class="panel-preview-btn"
              @click="openChartPreview($t('dashboard.statusDistributionPie'), 'statusPie')"
            >
              {{ $t("common.fullscreenPreview") }}
            </el-button>
          </div>
        ...
        <template #header>
          <div class="panel-header">
            ...
            <el-button
              ...
              size="small"
              class="panel-preview-btn"
              @click="openChartPreview($t('dashboard.throughputTrendCumulative'), 'throughput')"
            >
              {{ $t("common.fullscreenPreview") }}
            </el-button>
          </div>
        ...
        <template #header>
          <div class="panel-header">
            ...
            <el-button
              ...
              class="panel-preview-btn"
              @click="
                openChartPreview($t('dashboard.latencyDistributionComparison'), 'latencyDist')
              "
            >
              {{ $t("common.fullscreenPreview") }}
            </el-button>
      ...
      <el-card class="panel panel--top-routes" shadow="hover">
        <template #header>
          <div class="panel-header">
            ...
            <el-button
              ...
              size="small"
              class="panel-preview-btn"
              @click="openChartPreview($t('dashboard.topRouteRequestDistribution'), 'topRoutes')"
            >
              {{ $t("common.fullscreenPreview") }}
            </el-button>
          </div>
        ...
        <template v-else>
          ...
          <el-empty
            v-else
            :description="
              historicalData ? $t('dashboard.noRouteData') : $t('dashboard.realtimeNoDbHint')
            "
            :image-size="60"
          />
        </template>
...
const getListenAddrLabel = (listenAddr: string) =>
  isGlobalListen(listenAddr) ? t("dashboard.globalListen") : listenAddr;
...
const getPreviewOptionByKey = (key: typeof previewChartKey.value): EChartsOption => {
  switch (key) {
    case "qps":
      return qpsOption.value;
    case "status":
      return statusOption.value;
    case "latency":
      return latencyOption.value;
    case "percentile":
      return pOption.value;
    case "upstream":
      return upDistOption.value;
    case "rate":
      return rateOption.value;
    case "statusPie":
      return statusPieOption.value;
    case "throughput":
      return throughputOption.value;
    case "latencyDist":
      return latencyDistOption.value;
    case "topRoutes":
      return topRoutesOption.value;
    default:
      return {};
  }
};
...
const getRawWindowSeries = () => {
  ...
  const cut = <T>(arr: T[] | undefined, fallback: T) => {
    ...
    const padded = Array.from({ length: len }, () => fallback);
...
let lastValidView: { x: string[]; series: MetricsSeries; window: number; listen: string } | null =
  null;
...
const buildAlignedView = () => {
  ...
  if (!p) {
    ...
    if (
      lastValidView &&
      lastValidView.window === selectedWindow.value &&
      lastValidView.listen === selectedListen.value
    ) {
  ...
  const pick = <T>(arr: T[] | undefined, fallback: T): T[] => {
    ...
    if (tail.length !== tsTail.length) {
      const padded = Array.from({ length: tsTail.length }, () => fallback);
      const startAt = padded.length - tail.length;
      for (let i = 0; i < tail.length; i++) padded[startAt + i] = tail[i];
...
const avgLatency = computed(() => {
  ...
  if (historicalData.value && historicalData.value.avgLatencyMs.length > 0) {
    return Math.round(
      sum(historicalData.value.avgLatencyMs) / historicalData.value.avgLatencyMs.length,
    );
  }
  const raw = getRawWindowSeries();
...
const topRouteErr = computed(() => {
  ...
  if (
    ...
    historicalData.value.topRouteErr &&
    historicalData.value.topRouteErr.length > 0
  ) {
...
const topUpErr = computed(() => {
  ...
  if (
    historicalData.value &&
    historicalData.value.topUpErr &&
    historicalData.value.topUpErr.length > 0
  ) {
...
const formatPhaseMs = (v?: number) =>
  Number.isFinite(v as number) ? Number(v as number).toFixed(2) : "0.00";
...
const fetchTopRoutes = async () => {
  ...
  try {
    ...
    topUpstreamErrors.value = Array.isArray(res?.top_upstream_errors)
      ? res.top_upstream_errors.map((it) => ({
          item: String(it.item || ""),
  ...
  try {
    ...
    if (response && response.series) {
      const series: MetricsSeries = {
        ...
        upstreamDist: (response.series.upstreamDist || []).map((kv: any) => ({
          key: kv.key || kv.Key || "",
          value: kv.value || kv.Value || 0,
        })),
        topRouteErr: (response.series.topRouteErr || []).map((kv: any) => ({
          key: kv.key || kv.Key || "",
          value: kv.value || kv.Value || 0,
        })),
        topUpErr: (response.series.topUpErr || []).map((kv: any) => ({
          key: kv.key || kv.Key || "",
          value: kv.value || kv.Value || 0,
        })),
        latencyDist: (response.series.latencyDist || []).map((kv: any) => ({
          key: kv.key || kv.Key || "",
          value: kv.value || kv.Value || 0,
        })),
      };
...
const alignedView = computed(() => {
  ...
  if (historicalData.value) {
    ...
    const step =
      histSeries.timestamps.length > maxPoints
        ? Math.ceil(histSeries.timestamps.length / maxPoints)
        : 1;
    const idx: number[] = [];
  ...
  return {
    ...
    series: [
      ...
      },
      {
        ...
        largeThreshold: 200,
        data: s.s0,
        lineStyle: { width: 2, color: chartColors.value.gray },
        sampling: "lttb",
      },
    ],
  };
  ...
  return {
    ...
    series: [
      ...
      },
      {
        ...
        largeThreshold: 200,
        data: s.maxLatencyMs,
        lineStyle: { width: 2, color: chartColors.value.pink },
        sampling: "lttb",
      },
    ],
  };
  ...
  return {
    ...
    series: [
      ...
      },
      {
        ...
        largeThreshold: 200,
        data: s.p95 || [],
        lineStyle: { width: 2, color: chartColors.value.orange },
        sampling: "lttb",
      },
      {
        name: "p99",
...
const upDistOption = computed<EChartsOption>(() => {
  ...
  const data = upstreamDist
    .map((it) => ({ name: it.key, value: it.value }))
...
const rateOption = computed<EChartsOption>(() => {
  ...
  const err = s.s5xx.map(
    (v, i) => (Number.isFinite(v) ? v : 0) + (Number.isFinite(s.s0[i]) ? s.s0[i] : 0),
  );
  ...
  return {
    ...
    yAxis: {
      ...
      min: 0,
      max: 100,
      axisLabel: { color: chartColors.value.textMuted, formatter: (v: number) => `${v}%` },
      ...commonAxis.value,
    },
    series: [
      {
...
const statusPieOption = computed<EChartsOption>(() => {
  ...
  if (total2xx > 0)
    data.push({ name: "2xx", value: total2xx, itemStyle: { color: chartColors.value.success } });
  if (total3xx > 0)
    data.push({ name: "3xx", value: total3xx, itemStyle: { color: chartColors.value.info } });
  if (total4xx > 0)
    data.push({ name: "4xx", value: total4xx, itemStyle: { color: chartColors.value.warning } });
  if (total5xx > 0)
    data.push({ name: "5xx", value: total5xx, itemStyle: { color: chartColors.value.danger } });
  if (total0 > 0)
    data.push({
      name: t("dashboard.errorLabel"),
      value: total0,
      itemStyle: { color: chartColors.value.gray },
    });
...
const latencyDistOption = computed<EChartsOption>(() => {
  ...
  return {
    ...
    xAxis: {
      ...
      data: names,
      ...commonAxis.value,
      axisLabel: { ...commonAxis.value.axisLabel, rotate: 30 },
    },
    yAxis: { type: "value", ...commonAxis.value },
    series: [
      {
        ...
        type: "bar",
        data: vals,
        itemStyle: { color: chartColors.value.purple },
        barWidth: "60%",
      },
    ],
  };
...
const normalizeMetricsPayload = (payload: any): MetricsPayload => ({
  ...
  byListenAddr:
    payload?.byListenAddr && typeof payload.byListenAddr === "object" ? payload.byListenAddr : {},
  minuteWindowSeconds:
  ...
  topUpstreamErrors: Array.isArray(payload?.topUpstreamErrors)
    ? payload.topUpstreamErrors
    : undefined,
});
