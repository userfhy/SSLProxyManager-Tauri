...
const autoRefreshSeconds = document.getElementById(
  "auto-refresh-seconds",
) as HTMLInputElement | null;
const autoRefreshLabel = document.getElementById("auto-refresh-label") as HTMLElement | null;
const autoRefreshSecondsLabel = document.getElementById(
...
if (!currentPayload || !currentPayload.option) {
  ...
  chart.setOption(normalizeOptionForPreview(currentPayload.option), {
    notMerge: true,
    lazyUpdate: false,
  });
  ...
  const applyPayload = (payload: PreviewPayload) => {
    ...
    chart.setOption(normalizeOptionForPreview(payload.option), {
      notMerge: true,
      lazyUpdate: false,
    });
    applyPinnedTip();
  ...
  const requestLatestFromSource = async () => {
    ...
    if (
      ...
      setRefreshStatus(
        ok ? t("refreshedAt", { time: new Date().toLocaleTimeString() }) : t("refreshFailed"),
      );
      syncing = false;
      return;
    }
  ...
  chart.on("click", (params: any) => {
    ...
    if (pinnedTip && pinnedTip.seriesIndex === seriesIndex && pinnedTip.dataIndex === dataIndex) {
      pinnedTip = null;
  ...
  if ((window as any).__TAURI_INTERNALS__) {
    listen("chart-preview-opened-existing", (event) => {
      const payload = event?.payload as any;
